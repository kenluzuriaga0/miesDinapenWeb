<?php
    ...
    class Incidencia{
        ...
        public static function getAllIntervencionesByOperador($iDOperador) {
            $db = new Connection();
            require_once "../Personas/Personas.php";
            $query = "SELECT * FROM Intervenciones AS inte
            INNER JOIN ListaIDOperadores AS ope ON ope.IDOperador = inte.IDOperador
            INNER JOIN ListaIDOrganizacionesCoope AS org ON inte.IDOrganCooperante = org.IDOrganizacion 
            WHERE inte.IDOperador = ?
            ORDER BY inte.Estado desc, inte.FechaRegistro desc";

            $stmt = $db->prepare($query); // evitar SQL Injections
            $stmt->bind_param('i', $iDOperador);
            $stmt->execute();
        ...
        public static function getAllIntervencionesByCedula($cedula) {
            ...
            INNER JOIN ListaIDOperadores AS ope ON ope.IDOperador = inte.IDOperador
            INNER JOIN ListaIDOrganizacionesCoope AS org ON inte.IDOrganCooperante = org.IDOrganizacion 
            INNER JOIN ListaIDPersonas AS per ON inte.IDPersonaIntervenida = per.IDPersona 
            WHERE IDOperador.Cedula = ?
            ORDER BY inte.Estado desc, inte.FechaRegistro desc";
